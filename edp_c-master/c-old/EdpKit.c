...
int32 ReadFloat(EdpPacket* pkg, double* val)
{
    *val = 0;
    if (pkg->_read_pos + 4 > pkg->_write_pos)
        return -1;
    ...
    memcpy(val, pkg->_data + pkg->_read_pos, sizeof(double));
    pkg->_read_pos += 4;
    return 0;
}
...
int32 WriteFloat(Buffer* buf, double val)
{
    assert(buf->_read_pos == 0);
   ...
   /*此处假设机器为小端;
    ...
    return WriteBytes(buf, &val, sizeof(double));
}
